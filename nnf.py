...
def preprocess():
    ...
    mat = loadmat('/home/shiva/Downloads/basecode/mnist_all.mat') #loads the MAT object as a Dictionary
    
    #Pick a reasonable size for validation data
    fulltset=np.empty((0,784))
    ...
    a=range(fulltset.shape[0])
    aperm=np.random.permutation(a)
    A1=fulltset[aperm[0:50000],:]
    A2=fulltset[aperm[50000:],:]
    T1=trainlabel[aperm[0:50000]]
    T2=trainlabel[aperm[50000:]]
...
def nnObjFunction(params, *args):
    ...
    l1=np.zeros((training_label.shape[0],10));    

    for i in range (training_label.shape[0]):    
        l1[i][training_label[i]]=1;
        		
    training_data = np.append(training_data,np.ones([len(training_data),1]),1)
    n=training_data.shape[0]
    
    z,o=feedforward(training_data,w1,w2);

    y=l1
    err=y-o;
    scalar=np.square(err)
    delta=err*(1-o)*o
    grad2=-1*np.dot(delta.T,z)
    D=np.dot(delta,w2)
    z=z*(1-z)     
    D=D*z
    grad1=-1*np.dot(training_data.T,D)

    grad1 = np.delete(grad1, (n_hidden), axis=1)
    
    ecorr1=grad1.T+(lambdaval*w1);
    ecorr2=grad2+(lambdaval*w2);
    ...
    #obj_grad = np.array([])
    obj_grad = np.concatenate((err_grad1.flatten(), err_grad2.flatten()),0)
    obj_val= (1/2)*(np.sum(scalar)/n) + ( (lambdaval/(2 * n)) * ( np.sum(np.square(w1)) + np.sum(np.square(w2)) ) ) 
